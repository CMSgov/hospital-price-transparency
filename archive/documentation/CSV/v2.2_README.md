# Version 2.2 Hospital Price Transparency CSV Data Dictionary - effective January 1, 2025

Review this entire data dictionary for how to disclose data elements in CSV and find the [CSV "Tall" template here](./templates/V2.0.0_Tall_CSV_Format_Template.csv) and [CSV "Wide" template here](./templates/V2.0.0_Wide_CSV_Format_Template.csv) to begin building your hospital MRF. For an explanation of how to interpret the data element tables, review the [How to Read the Data Dictionary Tables](../../../documentation/HOW_TO_READ_DATA_DICTIONARY.md) information.
...
| **last_updated_on** | **last_updated_on** | MRF Date | Date | Date on which the MRF was last updated. Date must be in an ISO 8601 format (i.e. YYYY-MM-DD). See [additional last updated on notes](#additional-notes-on-last_updated_on) | No |
...
7. If a "payer specific negotiated charge" can only be expressed as a percentage or algorithm, then a corresponding "Estimated Allowed Amount" must also be encoded.
8. If code type is NDC, then the corresponding drug unit of measure and drug type of measure data element must be encoded.
...
10. If a value is encoded in the `drug_unit_of_measurement`, a value must also be encoded in the `drug_type_of_measurement`. Conversely, if a value is encoded in the `drug_type_of_measurement`, a value must be encoded in the `drug_unit_of_measurement`.
